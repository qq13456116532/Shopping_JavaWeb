...
public class ManagerServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		}else if("ReviseBooks".equals(operation)){
			ReviseBooks(request,response);
		}else if ("SingleBooks".equals(operation)){
			SingleBooks(request,response);
		}else if ("DeleteBook".equals(operation)){
			DeleteBook(request,response);
		}else if ("DeleteCategory".equals(operation)){
	...
	private void DeleteCategory(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		s.deleteCategory(request.getParameter("id"));
//		request.getRequestDispatcher("/manager/listCategory.jsp").forward(request, response);
		response.sendRedirect("/market/manager/listCategory.jsp");
	}

	private void DeleteBook(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		s.deleteBookById(request.getParameter("bookid"));
		request.getRequestDispatcher("/manager/listBooks.jsp").forward(request, response);
	}

	private void SingleBooks(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String id =request.getParameter("bookid");
		Book book = s.findBookById(id);
		request.setAttribute("book",book);
		request.getRequestDispatcher("/manager/ReviseBook.jsp").forward(request, response);
	}

	private void ReviseBooks(HttpServletRequest request, HttpServletResponse response) {
		Book book = WebUtil.findFormData(Book.class, request);
		s.addBook(book);
	}
